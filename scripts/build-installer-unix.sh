...
usage() {
    ...
    cat >&2 <<EOF
  Usage:
    $0 DAEDALUS-VERSION CRYPTOKAMI-BRANCH OPTIONS*
...
cryptokami_branch="$(printf '%s' "$1" | tr '/' '-')"; arg2nz "Cryptokami SL branch to build Daedalus with" $1; shift
...
CRYPTOKAMI_BUILD_UID="${OS_NAME}-${cryptokami_branch//\//-}"
ARTIFACT_BUCKET=ci-output-sink        # ex- cryptokami-sl-travis
CRYPTOKAMI_ARTIFACT=cryptokami-binaries     # ex- daedalus-bridge
CRYPTOKAMI_ARTIFACT_FULL_NAME=${CRYPTOKAMI_ARTIFACT}-${CRYPTOKAMI_BUILD_UID}

test -d node_modules/daedalus-client-api/ -a -n "${fast_impure}" || {
        retry 5 curl -o ${CRYPTOKAMI_ARTIFACT_FULL_NAME}.tar.xz \
              "https://s3.eu-west-1.amazonaws.com/${ARTIFACT_BUCKET}/cryptokami-sl/${CRYPTOKAMI_ARTIFACT_FULL_NAME}.tar.xz"
        mkdir -p node_modules/daedalus-client-api/
        du -sh  ${CRYPTOKAMI_ARTIFACT_FULL_NAME}.tar.xz
        tar xJf ${CRYPTOKAMI_ARTIFACT_FULL_NAME}.tar.xz --strip-components=1 -C node_modules/daedalus-client-api/
        rm      ${CRYPTOKAMI_ARTIFACT_FULL_NAME}.tar.xz
        echo "cryptokami-sl build id is $(cat node_modules/daedalus-client-api/build-id)"
        if [ -f node_modules/daedalus-client-api/commit-id ]; then echo "cryptokami-sl revision is $(cat node_modules/daedalus-client-api/commit-id)"; fi
        if [ -f node_modules/daedalus-client-api/ci-url ]; then echo "cryptokami-sl ci-url is $(cat node_modules/daedalus-client-api/ci-url)"; fi
        pushd node_modules/daedalus-client-api
              mv log-config-prod.yaml cryptokami-node cryptokami-launcher configuration.yaml *genesis*.json ../../installers
        popd
        chmod +w installers/cryptokami-{node,launcher}
        strip installers/cryptokami-{node,launcher}
        rm -f node_modules/daedalus-client-api/cryptokami-*
}
