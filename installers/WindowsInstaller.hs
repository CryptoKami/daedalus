...
launcherScript =
  ...
  , "start /D \"%DAEDALUS_DIR%\" cryptokami-launcher.exe " <> args
  ]
  where
    args = launcherArgs $ Launcher
      { nodePath = "%DAEDALUS_DIR%\\cryptokami-node.exe"
      , nodeLogPath = "%APPDATA%\\Daedalus\\Logs\\cryptokami-node.log"
...
writeUninstallerNSIS fullVersion = do
  ...
  writeFile "uninstaller.nsi" $ nsis $ do
    ...
    uninstall $ do
      ...
      mapM_ unsafeInject
        [ "liteFirewall::RemoveRule \"$INSTDIR\\cryptokami-node.exe\" \"Cryptokami Node\""
        , "Pop $0"
...
writeInstallerNSIS fullVersion = do
  ...
  writeFile "daedalus.nsi" $ nsis $ do
    ...
    _ <- section "" [Required] $ do
        ...
        file [] "cryptokami-node.exe"
        file [] "cryptokami-launcher.exe"
        ...
        mapM_ unsafeInject
          [ "liteFirewall::AddRule \"$INSTDIR\\cryptokami-node.exe\" \"Cryptokami Node\""
          , "Pop $0"
...
main = do
  ...
  echo "Adding permissions manifest to cryptokami-launcher.exe"
  procs "C:\\Program Files (x86)\\Windows Kits\\8.1\\bin\\x64\\mt.exe" ["-manifest", "cryptokami-launcher.exe.manifest", "-outputresource:cryptokami-launcher.exe;#1"] mempty

  signFile "cryptokami-launcher.exe"
  signFile "cryptokami-node.exe"
