...
main = do
  ...
  copyFile "cryptokami-launcher" (dir <> "/cryptokami-launcher")
  copyFile "cryptokami-node" (dir <> "/cryptokami-node")
  ...
  _ <- chain dir $ fmap T.pack [dir <> "/cryptokami-launcher", dir <> "/cryptokami-node"]
...
doLauncher :: String
doLauncher = "./cryptokami-launcher " <> (launcherArgs $ Launcher
  { nodePath = "./cryptokami-node"
  , walletPath = "./Frontend"
  , nodeLogPath = appdata <> "Logs/cryptokami-node.log"
